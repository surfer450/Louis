...
class Driver(ABC):
    ...
    def start_driver_action(self, input_queue: Queue, output_exchange: BaseExchange):
        self.is_recording = True
        self.thread = threading.Thread(target=self.driver_action_logic, args=(input_queue, output_exchange,)).start()

    def driver_action_logic(self, input_queue: Queue, output_exchange: BaseExchange) -> None:
        while True:
            ...
            if self.is_output_data_valid():
                output_exchange.publish(self.accumulator, "/")
